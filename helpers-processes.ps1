...
function Invoke-DetachedScriptLocally {
<#
.SYNOPSIS
...
#>
    ...
    $rootFolder = '\TempForDetachedScripts\'

    $resolvedScript = Resolve-Path -LiteralPath $ScriptPath -ErrorAction Stop
    $scriptBaseName = [IO.Path]::GetFileNameWithoutExtension($resolvedScript.Path)
    $randomHex = [guid]::NewGuid().ToString('N').Substring(0, 8)
    $TaskName = "$scriptBaseName-$randomHex"

    if ([string]::IsNullOrWhiteSpace($LogPath)) {
        $stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
        $LogPath = Join-Path $env:TEMP "$scriptBaseName-$stamp-$randomHex.log"
    }
    ...
    $quotedScript = '"' + $resolvedScript.Path.Replace('"', '\"') + '"'
    ...
    Start-ScheduledTask -TaskPath $rootFolder -TaskName $TaskName

    Write-Verbose "Deleting idle scheduled tasks under TaskPath '$rootFolder'. The newly created task '$TaskName' is excluded from this cleanup."

    $idleTasks = Get-ScheduledTask -TaskPath $rootFolder -ErrorAction SilentlyContinue |
        Where-Object {
    ...
    foreach ($task in $idleTasks) {
        Write-Verbose "Deleting idle scheduled task '$($task.TaskPath)$($task.TaskName)' with state '$($task.State)'."
...
#>
    ...
    if (Test-IsLocalComputerName -Name $Computer) {
        Write-Verbose "Computer '$Computer' is local. Using Invoke-DetachedScriptLocally."

        $baseDir = Join-Path $env:ProgramData 'TempForDetachedScripts'
        $scriptTempDir = Join-Path $baseDir 'Scripts'
        $logDir = Join-Path $baseDir 'Logs'

        if (-not (Test-Path -LiteralPath $scriptTempDir -PathType Container)) {
        ...
        Get-ChildItem -Path $scriptTempDir -Filter "$tempFilePrefix*.ps1" -File -ErrorAction SilentlyContinue |
        ...
        if ($PSCmdlet.ParameterSetName -eq 'ScriptBlock') {
            ...
            $localScriptPath = Join-Path $scriptTempDir $tempFileName

            Write-Verbose "Writing ScriptBlock to local SYSTEM-readable temp file: $localScriptPath"
        ...
        if ([string]::IsNullOrWhiteSpace($LogFile)) {
            $scriptBaseName = [IO.Path]::GetFileNameWithoutExtension($localScriptPath)
            $stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
            $randomHex = [guid]::NewGuid().ToString('N').Substring(0, 8)
            $LogFile = Join-Path $logDir "$scriptBaseName-$stamp-$randomHex.log"
        }

        $taskResult = Invoke-DetachedScriptLocally -ScriptPath $localScriptPath -LogPath $LogFile 

        [pscustomobject]@{
            ComputerName  = $env:COMPUTERNAME
            Status        = 'Success'
            ExecutionMode = 'LocalScheduledTask'
            ExecutionPath = $localScriptPath
            LogPath       = $taskResult.LogPath
            TaskPath      = $taskResult.TaskPath
            TaskName      = $taskResult.TaskName
            FullName      = $taskResult.FullName
            PowerShell    = $taskResult.PowerShell
            RunAs         = $taskResult.RunAs
            RunLevel      = $taskResult.RunLevel
        }
    ...
    try {
        ...
        Write-Verbose "Running cleanup job for old temporary script files on $Computer."
        ...
        $fileExists = Invoke-Command -Session $psSession -ScriptBlock {
            Test-Path -LiteralPath $using:remoteScriptPath
        }
        ...
        if ($localTempFileToCleanup -and (Test-Path -LiteralPath $localTempFileToCleanup)) {
            Write-Verbose "Cleaning up local temporary file: $localTempFileToCleanup"
            Remove-Item -LiteralPath $localTempFileToCleanup -Force -ErrorAction SilentlyContinue
        }
    }
    ...
    try {
        ...
        $r = Invoke-CimMethod -CimSession $cimSession -ClassName Win32_Process -MethodName Create -Arguments @{
            CommandLine = $cmd
        } -ErrorAction Stop

        if ($r.ReturnValue -ne 0) {
            $reason = if ($wmiReturnCodes.ContainsKey([int] $r.ReturnValue)) {
                $wmiReturnCodes[[int] $r.ReturnValue]
            }
            else {
                'Unknown WMI Error'
            }
        ...
        [pscustomobject]@{
            ...
            Status        = 'Success'
            ExecutionMode = 'RemoteWmiProcess'
